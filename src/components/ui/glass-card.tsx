  ...
  ({ className = "", eyebrow, metric, title, description, icon: Icon, ...props }, ref) => (
    ...
    >
      <div className="relative flex h-full min-h-[280px] flex-col overflow-hidden rounded-[24px] border border-black/[0.08] bg-white p-6 shadow-[0_18px_40px_-34px_rgba(23,29,51,0.34)] transition-[transform,border-color,box-shadow] duration-300 ease-out [transform-style:preserve-3d] group-hover:border-[#003aff]/20 group-hover:shadow-[0_24px_48px_-36px_rgba(0,58,255,0.34)] group-hover:[transform:rotateX(2deg)_rotateY(-3deg)_translateY(-4px)] motion-reduce:transform-none motion-reduce:transition-none md:p-8">
        <div className="product-format-icon-orbit pointer-events-none absolute right-0 top-0 h-40 w-40 [transform-style:preserve-3d]" aria-hidden="true">
          <span className="product-format-icon-ring-outer absolute right-[-56px] top-[-56px] h-40 w-40 rounded-full border border-[#003aff]/[0.08] bg-[#003aff]/[0.025] [transform:translate3d(0,0,8px)]" />
          <span className="product-format-icon-ring-inner absolute right-[-28px] top-[-28px] h-28 w-28 rounded-full border border-[#003aff]/[0.10] bg-white/70 [transform:translate3d(0,0,16px)]" />
          <span className="product-format-icon-circle absolute right-4 top-4 flex h-12 w-12 items-center justify-center rounded-full border border-[#003aff]/15 bg-white text-[#003aff] leading-none shadow-[0_10px_28px_-20px_rgba(0,58,255,0.55)] [transform:translate3d(0,0,24px)] transition-transform duration-300 group-hover:[transform:translate3d(0,0,32px)] motion-reduce:transform-none motion-reduce:transition-none">
            <Icon className="product-format-icon m-auto block h-5 w-5 shrink-0" strokeWidth={1.75} aria-hidden="true" />
          </span>
        </div>
