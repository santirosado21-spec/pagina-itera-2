...
export function ProductLedPage({ kinetic = false }) { return <PageShell className={`option option-one ${kinetic ? 'option-four' : ''}`}><Nav /><main id="main">
  <section className="lab-hero">{kinetic ? <KineticGrid className="lab-hero-kinetic" /> : <GradientBackground className="lab-hero-glow" />}<div className="container lab-hero-grid"><Reveal className="lab-copy"><p className="eyebrow">AI fluency you can measure</p><AnimatedHeroTitle /><p className="lead">Your team runs 15-minute simulations of the work they actually do. You see whose judgment holds up, where the gaps are, and what each person needs next.</p><div className="cta-row"><a className="button" href={links.demo}>Request a demo</a><a className="text-link" href="#product">See the product →</a></div></Reveal><Reveal className="workbench-wrap"><ProductWorkbench /></Reveal></div></section>
  ...
  <section className="measurement section"><div className="container grid gap-12 lg:grid-cols-[0.78fr_1.22fr] lg:items-start lg:gap-16"><div className="lg:sticky lg:top-28"><p className="eyebrow">Product format</p><h2>Small sessions. Defensible evidence.</h2><p className="citation max-w-[560px]">Organizations with high shadow-AI use averaged $670K more in breach costs — IBM, Cost of a Data Breach Report 2025.</p></div><div className="grid auto-rows-fr gap-4 sm:grid-cols-2">{productFormatCards.map(card => <GlassCard key={card.title} {...card} />)}</div></div></section>
  <Testimonials />
